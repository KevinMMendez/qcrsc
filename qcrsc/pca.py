...
def pca_plot(DataTable, PeakTable, pcx=1, pcy=2, batch='all', project_qc=True, transform='log', zero_remove=True, plot='all', control_limit=False, scale='unit', knn=3, colormap='Accent', color_batches=True, plot_ellipse='all', plot_points=True, scale_x=1, scale_y=1, fill_points=True, alpha_ellipse=(0.1, 0.1)):
    ...
    if len(batch) == 1:
        ...
        pca_multibatch(DataTable, PeakTable, pcx=pcx, pcy=pcy, batch=batch, project_qc=project_qc, transform=transform, zero_remove=zero_remove, plot=plot, control_limit=control_limit, scale=scale, knn=knn, colormap=colormap, color_batches=color_batches, plot_ellipse=plot_ellipse, plot_points=plot_points, scale_x=scale_x, scale_y=scale_y, fill_points=fill_points, alpha_ellipse=alpha_ellipse)
...
def pca_singlebatch(DataTable, PeakTable, pcx=1, pcy=2, batch='all', project_qc=True, transform='log', zero_remove=True, plot='all', control_limit=False, scale='unit', knn=3, colormap='Set3', color_batches=True, plot_ellipse='all', plot_points=True, scale_x=1, scale_y=1, fill_points=True, alpha_ellipse=(0.1, 0.1)):
    ...
    x = x.dropna(axis=1) # drop peaks with only NaNs
    t = BatchTable.Order
    qcw = BatchTable.QCW
    qcb = BatchTable.QCB
    qct = BatchTable.QCT
    ...
    text1 = Label(x=40, y=--2, x_units='screen', y_units='screen', text='No. of Peaks: {}'.format((len(x.T))), text_font_size='16pt')
...
def pca_multibatch(DataTable, PeakTable, pcx=1, pcy=2, batch='all', project_qc=True, transform='log', zero_remove=True, plot='all', control_limit=False, scale='unit', knn=3, colormap='Set3', color_batches=True, plot_ellipse='all', plot_points=True, scale_x=1, scale_y=1, fill_points=True, alpha_ellipse=(0.1, 0.1)):
    ...
    # Create batch_sampletype (if sample -> batch is value)
    batch_sampletype = []
    for i in range(len(sampletype)):
        if sampletype.loc[i] != 'Sample':
            batch_sampletype.append(sampletype[i])
        else:
            batch_sampletype.append(batch_list[i])
    ...
    unique_sampletype = np.unique(batch_sampletype)
    list_color = []
    for i in unique_sampletype:
        if i == 'Blank':
            list_color.append('#00FF00')
            ...
            b_rgb = colmap([int(i)])  # Sample Batch
            b_hex = matplotlib.colors.rgb2hex(b_rgb[0])
            list_color.append(b_hex)
    ...
    for i in range(len(unique_sampletype)):
        ...
        group_i_x = []
        group_i_y = []
        for j in range(len(batch_sampletype)):
            if str(batch_sampletype[j]) == unique_sampletype[i]:
                group_i_x.append(x_score_plot[j])
                group_i_y.append(y_score_plot[j])

        if isinstance(unique_sampletype[i], int) == True:
            alpha_ellipse_plot = alpha_ellipse[0]  # Sample
        else:
            alpha_ellipse_plot = alpha_ellipse[1]  # Not Sample
    ...
    # Clean later ... -> boxplot'
    group_sampletype = batch_sampletype
    group_label = group_sampletype
    cats = np.unique(group_label)
    ...
    # # Now create a control chart
    pc1 = pca_controlchart_multibatch(DataTable, PeakTable, x_score, batch, plot=plot, control_limit=control_limit, colormap=colormap, width=peak_width, height=peak_height, fill_point=fill_points, y_label="PC {} ({:0.1f}%)".format(pcx, x_expvar))

    pc2 = pca_controlchart_multibatch(DataTable, PeakTable, y_score, batch, plot=plot, control_limit=control_limit, colormap=colormap, width=peak_width, height=peak_height, fill_point=fill_points, y_label="PC {} ({:0.1f}%)".format(pcy, y_expvar))
...
def pca_controlchart_multibatch(DataTable, PeakTable, x_score, batch, plot='all', control_limit=False, colormap='plasma', y_label='y_label', width=10, height=10, fill_point=True):
    ...
    # MPA mean
    mpa_mean = np.nanmedian(x[qcb == 1])  # Need to edit
    ...
    if control_limit != False:
        for i in control_limit:
            for j in batch:
                ...
                low, upp = control_limits(x_j, qcb_j, sam_j, i, control_limit[i], False)
                ...
                if i == 'RSD':
                    ...
                    before_cl_dash = "dotdash"
                fig.line(x=t_j, y=before_cl_low, line_dash=before_cl_dash, line_width=2, line_color='darkblue')
                fig.line(x=t_j, y=before_cl_upp, line_dash=before_cl_dash, line_width=2, line_color='darkblue')
